...
export function entriesRouter(db) {
    ...
    // CREATE ENTRY
    // ============================================================
    const CreateEntry = EntryBase.refine((v) => v.vault_id || v.vaultId, { message: "vault_id o vaultId requerido" });
    ...
    const UpdateEntry = EntryBase
        ...
        .refine((v) => !v.vault_id || v.vaultId || v.vault_id, { message: "vault_id o vaultId requerido" });
    r.put("/entries/:id", (req, res) => {
        ...
        const e = parsed.data;
        const now = Date.now();
        const vault_id = e.vault_id ?? e.vaultId ?? existing.vault_id;
        const next = {
            vault_id,
